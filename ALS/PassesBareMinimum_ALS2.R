...
#Preparing for ALS Predictions

lb_feat<-read.csv("ALS2.csv")
leaderboard.predictions <- read.csv("ALS2_pred.csv")
...
lb_feat_median=temp[1:638,1:858]
...
lasso.pred=predict(lasso.mod,s=bestlam,newx=lb.x)
leaderboard.predictions$ALSFRS_slope=lasso.pred
write.csv(leaderboard.predictions, file = "ALS2_predictions.csv",row.names=FALSE)
